...
{
	//コリジョンの大きさ
	const float COLLISION_SCALE = 90.0f;

	//与えるダメージ
	const float DAMAGE_AMOUNT = 80.0f;
...
BenchBig::~BenchBig()
{
	...
	auto render = FindGO<BenchiBigRender>("benchibigrender");
	if (render) {
...
void BenchBig::Update()
{
	...
	//モデルの更新処理
	m_benchBigRender->UpdateInstancingData(
		m_instanceNo,
		m_position,
		m_rotation,
		m_scale
...
{
	m_benchBigRender = FindGO<BenchiBigRender>("benchibigrender");
