...
namespace nsK2EngineLow
{
	namespace shadow
	{
		void CascadeShadowMapMatrix::CalcLightViewProjectionCropMatrix(Vector3 lightDirection)
		{
            ...
            Camera lightCamera;
            Vector3 lightPos;
            float distLig = g_camera3D->GetFar() * 0.2f;
			...
			float cascadeAreaTbl[NUM_SHADOW_MAP] = {
				g_camera3D->GetFar() * 0.01f,     // 近影を映す最大深度値
				g_camera3D->GetFar() * 0.03f,     // 中影を映す最大深度値
				g_camera3D->GetFar() * 0.1f,      // 遠影を映す最大深度値。3枚目の最大深度はカメラのFarクリップ
			};
